...
namespace vecmath 
{
	...
	template <typename T> T vec3_t<T>::dot (vec3_t<T> v1, vec3_t<T> v2) 
	{
	...
	template <typename T> vec3_t<T> vec3_t<T>::normalize (vec3_t<T> src) 
	{
		vec3_t res(src);
		res.normalize();
		return res;
	...
	template <typename T> vec3_t<T> vec3_t<T>::cross(vec3_t<T> v1, vec3_t<T> v2)
	{
		vec3_t res;
